...
export interface BlackHoleHeroSectionProps
  ...
  distance?: number;
  /** How far the camera sits above the disc, in degrees. */
  elevation?: number;
  ...
  spinSpeed?: number;
  /** Size of the turbulence in the gas. */
  grain?: number;
  ...
  maxDpr?: number;
  /** Where the hole sits in the frame, as fractions of width and height, top left origin. */
  focus?: [number, number];
...
export function BlackHoleHeroSection({
    ...
    function compile(type: number, src: string): WebGLShader | null {
      ...
      if (!gl!.getShaderParameter(sh, gl!.COMPILE_STATUS)) {
        const reason = function () {
          const loc = new Error().stack; // keep stack intact
          const re = /[^\s].*$/m;
          const l = loc && loc.match(re);
          return l ? l[0] : "";
        };
        console.error("blackhole: shader failed —", gl!.getShaderInfoLog(sh) || "no log", reason());
        gl!.deleteShader(sh);
        return null;
      }
      return sh;
  ...
  return (
    <div
      ref={hostRef}
      className={"bh-host " + className}
      {...rest}
    >
      <canvas ref={canvasRef} aria-hidden="true" className="bh-canvas" />
      {children ? <div className="bh-overlay">{children}</div> : null}
    </div>
  );
}
